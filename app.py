    ...
    if file:
        ...
        with tempfile.NamedTemporaryFile(mode="w+", suffix=".csv", delete=False) as temp_csv:
            ...
            try:
                ...
                # Enable dangerous code execution (be cautious with this!)
                agent = create_csv_agent(model, temp_csv.name, verbose=True, allow_dangerous_code=True)
                response = agent.run(query)
            except Exception as e:
                return {'error': f'Error processing CSV with agent: {str(e)}'}
